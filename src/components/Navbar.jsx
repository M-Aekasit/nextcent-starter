    ...
    <nav className="bg-[#F5F7FA]">
      <div className="container mx-auto max-w-[1320px] relative h-auto p-10 flex flex-col md:flex-row md:justify-between md:items-center md:h-[80px]">
        ...
        <ul className={`${!toggle ? 'hidden' : 'flex'} flex flex-col md:flex md:flex-row my-5`}>
          <li className="my-2 mx-4">
            <a
              className="inline-flex justify-center items-center py-2 px-4 text-[#4CAF4F] "href="#">
              Login
            </a>
          </li>
          <li className="my-2 mx-4">
            <a
              className="inline-flex justify-center items-center py-2 px-4 bg-[#4CAF4F] text-white rounded-md" href="#">
              Sign Up
            </a>
          </li>
        </ul>
        {/* Hamburger Icon */}
        <FaBars onClick={updateToggle} className="absolute right-5 cursor-pointer text-xl" />
      </div>
    </nav>
  );
}
